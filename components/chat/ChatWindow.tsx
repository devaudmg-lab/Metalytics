...
export default function ChatWindow({ lead, messages, onSendMessage, isLoading, onBack, platform }: ChatWindowProps) {
        ...
        <div className="flex-1 flex flex-col h-full bg-[#f0f2f5] dark:bg-[#0a0a0a] relative">
            ...
            <div className="p-3 md:p-4 bg-white dark:bg-[#080808] border-b border-gray-200 dark:border-white/5 flex items-center justify-between shadow-sm z-10">
                <div className="flex items-center gap-2 md:gap-3">
                    ...
                    <div className={`w-10 h-10 rounded-full ${isWhatsAppSource ? 'bg-emerald-600' : 'bg-blue-600'} text-white flex items-center justify-center text-lg font-bold shadow-inner`}>
                    ...
                    <div>
                        ...
                        {/* Show platform tag */}
                        <p className={`text-[10px] md:text-[11px] font-medium mt-0.5 ${isWhatsAppSource ? 'text-emerald-500' : 'text-blue-500'}`}>
                ...
                {lead.phone && (
                    ...
                    >
                        <Phone size={14} /> <span className="hidden sm:inline">WhatsApp Call</span>
                    </button>
                )}
            </div>
                        ...
                        <div key={msg.id} className={`flex ${msg.sender === 'user' ? "justify-start" : "justify-end"}`}>
                            <div className={`max-w-[85%] md:max-w-[75%] px-3 py-2 rounded-xl text-[14px] shadow-sm ${
                                msg.sender === 'user'
                                    ...
                                    : `${isWhatsAppSource ? 'bg-emerald-600' : 'bg-blue-600'} text-white rounded-br-none`
                            }`}>
                ...
                {canReply ? (
                    <form onSubmit={handleSend} className="flex gap-2 items-center">
                        <input
                            ...
                            placeholder={isWhatsAppSource ? "Type a WhatsApp message..." : "Type a Messenger message..."}
                            value={inputText}
                        ...
                        <button
                            ...
                            className={`w-10 h-10 rounded-full flex items-center justify-center text-white active:scale-95 transition-all disabled:bg-gray-300 ${
                                isWhatsAppSource ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-blue-600 hover:bg-blue-700'
                            }`}
                        >
                    ...
                    <div className="py-2 text-center text-[11px] text-orange-500 bg-orange-50 dark:bg-orange-500/10 rounded-lg">
                        This lead source ({lead.source}) does not support direct replies yet.
                    </div>
                )}
            </div>
        </div>
